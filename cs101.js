...
function addCustomData (filename, data) {
    ...
    globalCustomFiles[filename.toLowerCase()] = data;
}
...
function getCustomData (filename) {
    ...
    if (filename in globalCustomFiles) {
        console.log('filename found in custom data:' + filename);
        return globalCustomFiles[filename];
    }
    else {
        // BUGBUG: handle properly
        //alert('problem getting data:' + filename);
        return null;
    }
}
    ...
    for (var i = 0, f; f = files[i]; i++) {
        if (f.type.match(type)) {
            ...
            reader.onload = (function(theFile) {
                return function(e) {
                    addCustomData(theFile.name, e.target.result);
...
function toggleImageView () {
	...
	//}
	var imgData = setSize(canvas, newWidth, newHeight);
    canvas.width = newWidth;
    canvas.height = newHeight;
    canvas.getContext("2d").putImageData(imgData, 0, 0, 0, 0, newWidth, newHeight);
}
...
function evaluateInput (text) {
    ...
    window.globalEvalFn = function() { evaluateShow(text); };
...
function getFileURL (filename) {
    var custom = getCustomData(filename);
    if (custom) {
...
function extractImages (code) {
	...
	if (result.length == 0) {
    	// hardcode the image used for sized images is pre-loaded
    	result.push(globaleEmptyImageLocation);
	}
	return result;
}
...
function preloadImages (names) {
    // 2014 special case: if no images to load, jump to running the "post" code.
    if (names.length == 0 && window.globalEvalFn) {  // 2015
        window.globalEvalFn();
    }
    else {
...
function loadImage (filename) {
    return makeHTMLImage(getFileURL(filename));
}
...
function postImage (img) {
    ...
    if (window.globalImageCount == window.globalImageNeeded && window.globalEvalFn) { // 2015
        window.globalEvalFn();
	}
}
...
function makeHTMLImage (filename) {
    var id = 'img' + window.globalImageCount++;
    console.log('load image:' + id + ' ' + filename);
    // append img tag
    var output = getOutput();
    var img = new Image();
    ...
    img.setAttribute('onError','postImage(this);');
